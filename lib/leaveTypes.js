...
export const LEAVE_TYPES = [
    ...
    { value: 'Unpaid Leave', label: 'Unpaid Leave', key: 'unpaidLeave' },
...
export const LEAVE_BALANCE_TYPES = [
    ...
    { key: 'unpaidLeave',    label: 'Unpaid Leave',    bgColor: 'bg-red-900/30',     textColor: 'text-red-300',     titleColor: 'text-red-200' }
];

// Leave types that are only offered when the user has a positive balance for them.
// Leave in-lieu and Unpaid Leave are intentionally excluded: they are not
// pre-allocated balances (in-lieu is earned per substitution, unpaid is the
// fallback offered when other balances run out) so they stay available regardless.
const BALANCE_GATED_KEYS = ['annualLeave', 'casualLeave', 'sickLeave', 'shortLeave', 'maternityLeave', 'paternityLeave', 'other'];

// Helper functions
export const getFilteredLeaveTypes = (gender, leaveBalances) => {
    return LEAVE_TYPES.filter(leaveType => {
        if (leaveType.gender && leaveType.gender !== gender) {
            return false;
        }
        if (leaveBalances && BALANCE_GATED_KEYS.includes(leaveType.key)) {
            return (leaveBalances[leaveType.key] || 0) > 0;
        }
        return true;
    });
};
