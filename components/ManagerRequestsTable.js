                ...
                <table className="min-w-full divide-y divide-gray-700">
                    <thead className="bg-muted">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Requirements</th>
                    ...
                    <tbody className="bg-card divide-y divide-gray-700">
                        {requests.map(req => (
                            <tr key={req.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    {req.type === 'Sick Leave' && req.medicalDocumentRequired ? (
                                        <div className="flex items-center space-x-1">
                                            <svg className="w-4 h-4 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                            <span className="text-xs text-red-400 font-medium">Medical Doc Required</span>
                                        </div>
                                    ) : req.type === 'Unpaid Leave' ? (
                                        <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-900/30 text-red-400">
                                            HR Approval
                                        </span>
                                    ) : (
