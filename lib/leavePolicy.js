...
export const calculateLeaveBalances = (employee, leaveRequests = [], year = new Date().getFullYear()) => {
    ...
    // Helper to convert Firestore Timestamp or string/Date to a JS Date
    const toDate = (val) => {
        if (!val) return null;
    ...
    const approvedRequests = leaveRequests.filter(req => {
        const startDate = toDate(req.startDate);
        return req.userId === employee.uid &&
            req.status === 'Approved' &&
            startDate !== null &&
...
export const getCurrentMonthShortLeaveUsage = (userId, leaveRequests) => {
    ...
    return leaveRequests.reduce((total, req) => {
        ...
        // Filter: Current Month Only - handle Firestore Timestamp or string
        let reqDate;
        if (req.startDate && typeof req.startDate === 'object' && typeof req.startDate.toDate === 'function') {
            reqDate = req.startDate.toDate();
        } else {
